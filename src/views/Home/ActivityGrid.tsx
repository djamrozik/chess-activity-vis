...
const getGameCount = (dateToGameCount: DateToGameCount | undefined, d: Date) => {
  if (!dateToGameCount) {
    return 0;
  }
  const fullYear = String(d.getFullYear());
  const fullMonth = d.getMonth() < 9 ? `0${d.getMonth() + 1}` : `${d.getMonth() + 1}`;
  const day = d.getDate();
...
const ActivityGrid = ({ dateToGameCount, gridDates, state, errorMessage }: ActivityGridProps) => {
      ...
      <div className="flex flex-row-reverse overflow-x-scroll">
        <div>
          <div
            ...
            style={{
              gridTemplateColumns: `${cellWidth}px `.repeat(cols - 1) + `${cellWidth}px`,
            }}
          >
          ...
          >
            {gridDates.map((d, i) => (
              <GridCell key={i} id={i} gameCount={getGameCount(dateToGameCount, d)} date={d} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      <div className="h-[32px] pb-2 pl-1 text-sm">
        {state === "loaded" && (
          <div>
            {totalGamesPlayed.toLocaleString()} {totalGamesPlayed !== 1 ? "games" : "game"} played in the last year
          </div>
        )}
      </div>
      <div className="rounded-lg border border-[var(--border-primary)] p-4 dark:border-[var(--border-primary-dark)]">
        {Boolean(state === "loaded" && !errorMessage) && renderInnerGridContent()}
        {Boolean(state === "toSearch" || state === "loading" || errorMessage) && (
          <div className="flex w-full">
            <div className="flex h-[123px] w-[829px] items-center justify-center">
              {errorMessage && <div className="max-w-sm text-center text-red-500">{errorMessage}</div>}
              {!errorMessage && <div>{state === "loading" ? "Loading..." : "Activity grid will be shown here"}</div>}
            </div>
          </div>
        )}
      </div>
